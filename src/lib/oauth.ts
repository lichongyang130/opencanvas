...
export function oauthAuthorizeUrl(cfg: OAuthProviderConfig, state: string): string {
...
export async function exchangeOAuthCode(
  ...
  if (!res.ok || !data.access_token) {
    throw new Error(data.error_description || data.error || `换取令牌失败 ${res.status}`);
  }
...
export async function fetchOAuthProfile(
  ...
  if (cfg.provider === "google") {
    ...
    if (!res.ok) throw new Error(`获取 Google 资料失败 ${res.status}`);
  ...
  if (!res.ok) throw new Error(`获取 GitHub 资料失败 ${res.status}`);
