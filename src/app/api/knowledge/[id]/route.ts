...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  const cur = repo.getKnowledgeBase(params.id);
  ...
  repo.updateKnowledgeBase(params.id, {
  ...
  return Response.json({ base: repo.getKnowledgeBase(params.id) });
}
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  const cur = repo.getKnowledgeBase(params.id);
  if (!cur) return Response.json({ error: "知识库不存在" }, { status: 404 });
  repo.deleteKnowledgeBase(params.id);
