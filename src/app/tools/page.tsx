          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="flex items-center gap-1.5">
                <span className="block text-[13.5px] font-semibold text-stone-800">{tt(t.name)}</span>
                {t.kind === "soon" && (
                  <span className="rounded bg-stone-100 px-1.5 py-0.5 text-[9.5px] font-medium text-stone-400">{tt("规划中")}</span>
                )}
                {t.kind === "link" && <ArrowUpRight className="h-3.5 w-3.5 text-stone-300" />}
              </span>
              <span className="mt-1 block text-xs leading-5 text-stone-400">{tt(t.desc)}</span>
            </span>
          </button>
        ))}
      </div>
...
export default function ToolsPage() {
  ...
  const onUse = async (t: Tool) => {
    ...
    if (t.kind === "chat" && t.prompt) {
      ...
      toast(tt("已打开对话框并预填「{name}」指令，粘贴内容后发送即可", { name: tt(t.name) }), "success");
      router.push("/chat");
      return;
    }
    if (t.kind === "soon") {
      toast(t.soonNote ?? tt("该工具正在规划中"), "info");
    }
  };
        ...
        <header className="flex shrink-0 items-center justify-between border-b border-[var(--oc-border-soft)] bg-[var(--oc-bg)] px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-[12.5px] text-stone-400">
              {tt("能真实执行的工具已接真；「规划中」为需要专业后端能力的项目")}
            </p>
          </div>
          ...
          <div className="mx-auto w-full max-w-[1180px]">
            ...
            <div className="flex items-center gap-2 rounded-xl border border-[var(--oc-border)] bg-white px-3.5 py-2.5 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
              ...
              <input
                ...
                placeholder={tt("搜索 26 个工具（如：研究、视频、翻译、PDF）")}
              ...
              <>
                ...
                {filtered.length === 0 && (
                  <div className="mt-4 rounded-2xl border border-[var(--oc-border-soft)] bg-white px-6 py-12 text-center text-stone-400">
                    ...
                    <p className="mt-2 text-[13px]">{tt("没有找到匹配的工具")}</p>
                    <p className="mt-1 text-[11.5px]">{tt("试试「写作」「翻译」「PDF」等关键词")}</p>
                  </div>
                )}
              </>
            ) : (
              <>
                <ToolGrid title={tt("文档处理")} tools={DOC_TOOLS} onUse={onUse} />
                <ToolGrid title={tt("内容创作（基于 AI 对话引擎）")} tools={CONTENT_TOOLS} onUse={onUse} />
                <ToolGrid title={tt("数据处理")} tools={DATA_TOOLS} onUse={onUse} />
                <div className="mt-5 flex items-center justify-between">
                  <h2 className="text-[15px] font-semibold text-stone-800">
                    {tt("协作工具")}
                    <span className="ml-2 text-[12px] font-normal text-stone-400">{tt("需要团队版账号，当前单机版未开放")}</span>
                  </h2>
                </div>
