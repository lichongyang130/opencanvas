          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[12.5px] text-stone-400">
                输入描述生成 3 秒可循环短片；当前为内置演示引擎（零密钥可用），真实模型接入后可切换
              </p>
            </div>
          </div>
          ...
          <div className="mx-auto grid w-full max-w-[1100px] grid-cols-1 gap-5 lg:grid-cols-[380px_1fr]">
            ...
            <div className="flex flex-col gap-4">
              <section className="rounded-2xl border border-[var(--oc-border)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
                <label className="text-[13px] font-semibold text-stone-700">视频描述</label>
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  maxLength={500}
                  rows={5}
                  placeholder="例如：海浪拍打礁石，金色夕阳下，4K 电影感"
                ...
                <div className="mt-1 flex items-center justify-between text-[11px] text-stone-400">
                  <span>支持画面风格、主体、氛围、镜头描述</span>
                  <span>{prompt.length}/500</span>
                </div>

                <p className="mt-3 text-[11.5px] font-medium text-stone-400">试试：</p>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Sparkles className="h-4 w-4" /> 生成视频（免费演示）
                    </>
                  )}
                </button>
              ...
              <section className="rounded-2xl border border-[var(--oc-border)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
                <h2 className="flex items-center gap-1.5 text-[13px] font-semibold text-stone-700">
                  <Film className="h-4 w-4 text-stone-400" /> 生成引擎
                </h2>
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-stone-400">
                          {m.providerLabel}
                          {m.creditsPerVideo > 0 ? ` · ${m.creditsPerVideo} 积分/次` : " · 免费"}
                        </p>
                      </div>
                      <span
                      ...
                      >
                        {m.provider === "demo" ? "可用" : "待接入"}
                      </span>
                    </li>
                  ))}
                  {models.length === 0 && (
                    <li className="text-[12px] text-stone-400">引擎状态加载中…</li>
                  )}
                </ul>
                <p className="mt-3 text-[11px] leading-5 text-stone-400">
                  演示引擎在本机合成程序化动画，无需任何外部 Key；FAL / 万相视频将复用同一前端与计费通道。
                </p>
              </section>
            </div>
            ...
            <section className="flex min-h-[420px] flex-col rounded-2xl border border-[var(--oc-border)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
              ...
              <div className="mt-4 flex flex-1 items-center justify-center overflow-hidden rounded-xl bg-[#0e1116] p-4">
                {loading && (
                  <div className="text-center text-stone-500">
                    ...
                    <p className="mt-3 text-[12.5px]">正在合成动画帧…</p>
                  </div>
                )}
                {!loading && !result && !error && (
                  <div className="text-center text-stone-500">
                    <Clapperboard className="mx-auto h-10 w-10 text-stone-600" />
                    <p className="mt-3 text-[13px]">生成后可在此预览并下载</p>
                ...
                {!loading && result && (
                  <div className="w-full">
                    ...
                    <div className="mt-3 flex justify-center gap-2">
                      ...
                      >
                        <RefreshCw className="h-3.5 w-3.5" /> 换一个描述
                      </button>
                      <button
                        onClick={download}
                      ...
                      >
                        <Download className="h-3.5 w-3.5" /> 下载 GIF
                      </button>
                    </div>
                  </div>
                )}
              ...
              <p className="mt-4 text-[11px] leading-5 text-stone-400">
                演示模式为程序化动画（GIF），无外部生成成本；真实视频模型接入后将在此展示 MP4 并扣除对应积分。
              </p>
            </section>
          </div>
        </div>
